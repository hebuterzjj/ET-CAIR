    ...
    for bi = 1:block_h
        for bj = 1:block_w           
            ...
            CSet_Ret = [CBlock_Func_aprox_X(:)...
                CBlock_Func_aprox_Y(:)]; 
            % the point set in  img mapretped from the regular block in org img
            CSet_Ret(CSet_Ret(:,1) == 0,:) = [];
            CSet_Ret(CSet_Ret(:,2) == 0,:) = [];
            ...
            C = sum(CBlock_Func_aprox_Y~=0,2);
            D = min(C);
            
            Block_change_info(bi, bj,1) = (X_MAX_ret-X_MIN_ret+1);
            Block_change_info(bi, bj,2) = D;
        end
    end  
