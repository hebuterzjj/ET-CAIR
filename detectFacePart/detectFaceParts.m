...
%                                                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [bbox,bbX,faces,bbfaces,ourface] = detectFaceParts(detector,X,thick)
...
if( nargout > 2 )
 ...
 if(size(bbox,1)==0)
     ourface=zeros(size(X,1),size(X,2)) ;  
 end
  ourface=zeros(size(X,1),size(X,2)) ;
 for i=1:size(bbox,1)
  
  ourface(bbox(i,2):bbox(i,2)+bbox(i,4)-1,bbox(i,1):bbox(i,1)+bbox(i,3)-1,:)=0.8;
%    figure();imshow(ourface);
  faces{i,1} = X(bbox(i,2):bbox(i,2)+bbox(i,4)-1,bbox(i,1):bbox(i,1)+bbox(i,3)-1,:);
%    figure(),imshow( faces{i,1});
